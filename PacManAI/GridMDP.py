...
class GridMDP(MDP):
    def __init__(self, grid, terminals, init=(0, 0), gamma=.9):
        ...
        for x in range(self.cols):
            self.visits.append(list())
            for y in range(self.rows):
                self.visits[x].append(1)
    ...
    def DrawGrid( self, gridworld ):
        ...
        #             plt.plot([x*CSTEP, (x+1)*CSTEP], [y*RSTEP, (y+1)*RSTEP], zorder=1, color='k', linestyle='-', linewidth=0.5)
        #             plt.plot([(x+1)*CSTEP, (x)*CSTEP], [(y)*RSTEP, (y+1)*RSTEP], zorder=1, color='k', linestyle='-', linewidth=0.5)
    ...
    def DrawVs( self, gridworld, V ):
        ...
        for k, v in V.items():
            ...
            t = plt.text(x+CSTEP/2-CSTEP/8, y+RSTEP/2-0*RSTEP/16, '%2.2f' % (v), fontsize=vfontsize)
    ...
    def DrawQs( self, gridworld, Qs ):
        ...
        for k, v in Qs.items():
            ...
            if k == gridworld.terminals[0][0] or k == gridworld.terminals[1][0]:
                t = plt.text(x+CSTEP/2-CSTEP/8, y+RSTEP/2-0*RSTEP/16, '%2.2f' % (v[0]), fontsize=qfontsize)
                t.set_bbox(dict(facecolor='white', alpha=0.9, edgecolor='white'))
            else:
                # left
                t=plt.text(x+(CSTEP/2-d), y+RSTEP/2, '%2.2f' % (v[2]), color='grey', fontsize=qfontsize)
                t.set_bbox(dict(facecolor='white', alpha=0.9, edgecolor='white'))
                # right
                t=plt.text(x+(CSTEP/2+d/2), y+RSTEP/2, '%2.2f' % (v[0]), color='grey', fontsize=qfontsize)
                t.set_bbox(dict(facecolor='white', alpha=0.9, edgecolor='white'))
                # bottom
                t=plt.text(x+(CSTEP/2-0.1*CSTEP), y+(RSTEP/2-d/2), '%2.2f' % (v[3]), color='grey', fontsize=qfontsize)
                t.set_bbox(dict(facecolor='white', alpha=0.9, edgecolor='white'))
                # top
                t=plt.text(x+(CSTEP/2-0.1*CSTEP), y+(RSTEP/2+d/2), '%2.2f' % (v[1]), color='grey', fontsize=qfontsize)
                t.set_bbox(dict(facecolor='white', alpha=0.9, edgecolor='white'))
    ...
    def DrawQ( self, gridworld, s, a, Q ):
        ...
        if s == gridworld.terminals[0][0] or s == gridworld.terminals[1][0]:
            t = plt.text(x+CSTEP/2-CSTEP/8, y+RSTEP/2-0*RSTEP/16, '%2.2f' % (Q), fontsize=qfontsize)
            t.set_bbox(dict(facecolor='white', alpha=0.9, edgecolor='white'))
            return

        if a == 0:
            # right
            t=plt.text(x+(CSTEP/2+d/2), y+RSTEP/2, '%2.2f' % (Q), color='grey', fontsize=qfontsize)
            t.set_bbox(dict(facecolor='white', alpha=0.9, edgecolor='white'))
        elif a == 1:
            # top
            t=plt.text(x+(CSTEP/2-0.1*CSTEP), y+(RSTEP/2+d/2), '%2.2f' % (Q), color='grey', fontsize=qfontsize)
            t.set_bbox(dict(facecolor='white', alpha=0.9, edgecolor='white'))
        elif a == 2:
            # left
            t=plt.text(x+(CSTEP/2-d), y+RSTEP/2, '%2.2f' % (Q), color='grey', fontsize=qfontsize)
            t.set_bbox(dict(facecolor='white', alpha=0.9, edgecolor='white'))
        elif a == 3:
            # bottom
            t=plt.text(x+(CSTEP/2-0.1*CSTEP), y+(RSTEP/2-d/2), '%2.2f' % (Q), color='grey', fontsize=qfontsize)
    ...
    def DrawArrows( self, gridworld, pi ):
        ...
        for k, v in pi.items():
            ...
            y = k[1]*RSTEP
            # top
            points = [[x+(CSTEP/2 - 0.1*CSTEP), y+(RSTEP-0.1*RSTEP)], [x+(CSTEP/2 + 0.1*CSTEP), y+(RSTEP-0.1*RSTEP)], [x+CSTEP/2, y+RSTEP]]
            triangle = plt.Polygon(points, fc='white')
            plt.gca().add_patch(triangle)
            # bottom
            points = [[x+(CSTEP/2 - 0.1*CSTEP), y+(0.1*RSTEP)], [x+(CSTEP/2 + 0.1*CSTEP), y+(0.1*RSTEP)], [x+CSTEP/2, y]]
            triangle = plt.Polygon(points, fc='white')
            plt.gca().add_patch(triangle)
            # left
            points = [[x+(0.1*CSTEP), y+(RSTEP/2-0.1*RSTEP)], [x+(0.1*CSTEP), y+(RSTEP/2+0.1*RSTEP)], [x, y+RSTEP/2]]
            triangle = plt.Polygon(points, fc='white')
            plt.gca().add_patch(triangle)
            # right
            points = [[x+(CSTEP - 0.1*CSTEP), y+(RSTEP/2-0.1*RSTEP)], [x+(CSTEP - 0.1*CSTEP), y+(RSTEP/2+0.1*RSTEP)], [x+CSTEP, y+RSTEP/2]]
            triangle = plt.Polygon(points, fc='white')
            plt.gca().add_patch(triangle)
